...
function calEEXI(){
    var selval = $("#shiptype option:selected").val();
    if(selval == ""){
      alert("select Ship Type");
      return;
    }
    var chkfuel = $('input:checkbox[name="fueltype"]:checked').length;
    var select_obj='';
    ...
    alert(selval+","+chkfuel+":"+select_obj);
}
...
function searchRoute_db(){
  ...
  xhttp.onreadystatechange = function(){
    if (xhttp.readyState == 4 && xhttp.status == 200) {
      ...
      for(var i=0; i<obj.length; i++){
        ...
        tablehtml = tablehtml +
                    `<tr>
                      ...
                      <td>`+obj[i].lng+`</td>
                      <td>`+crsnext+`</td>
                      <td>`+(obj[i].distnext/1.8).toFixed(1)+`</td>
                      <td>`+(obj[i].accdist/1.852).toFixed(1)+`</td>
                      <td>`+((ttldist-obj[i].accdist)/1.8).toFixed(1)+`</td>
                      <td>`+(((ttldist-obj[i].accdist)/1.8)/$('#speed').val()).toFixed(2)+`</td>
                      </tr>`;
      }
      ...
      document.getElementById("eta").innerHTML = (totaldist/$('#speed').val()).toFixed(2)+" hours";
...
function calUKC(){
  ...
  $.ajax({
    type: 'GET',
    url: "http://192.168.1.130:5000/getukc"+queryData,
    data: "test",
    dataType: 'json',
    success: function(data){
      ...
      <div class="tab-content" id="myTabContent">
        <div class="tab-pane fade in active" id="open" role="tabpanel" aria-labelledby="open-tab">
          <div style="width:33%;float:left;border:1px solid gold;">
            <strong>Calculationof expected UKC</strong>
            <table  class="table tb-border">
                ...
                <tr>
                    <td><font color=blue>Anticipated controlling depth</font></td>
                    <td id="sumdepth"><font color=blue><strong>`+data.opensea[0].contdepth.toFixed(2)+`</strong></font></td>
                </tr>
                <tr>
                    <td><font color='blue'>Calculated Maximum Draft</td>
                    <td id="maxdraft"><font color=blue><strong>`+data.opensea[0].maxdraft.toFixed(2)+`</strong></font></td>
                </tr>
                <tr>
                    <td><font color="red">Calculated  UKC</font></td>
                    <td id="calUKC"><font color=red><strong>`+data.opensea[0].calUKC.toFixed(2)+`</strong></font></td>
                </tr>
            </table>
            <button type="button" class="ukc-btn">Edit</button>
          </div>
          <div style="width:33%;float:left;">
            <strong>Anticipated controlling Depth</strong>
            <table  class="table tb-border">
              <colgroup>
                <col width="80%">
                <col width="30%">
              ...
              <tr>
                  <td><small><font color=blue>Anticipated controlling depth</font></small></td>
                  <td id="condepth"><font color=blue>`+data.opensea[0].contdepth.toFixed(2)+`</font></td>
              </tr>
            </table>
          </div>
          <div style="width:33%;float:left;">
            <strong>Calculated Maximum  Draft</strong>
            <table  class="table tb-border">
                 <colgroup>
                    <col width="40%">
                    <col width="20%">
                ...
                <tr>
                    <td><small><font color='blue'>Cal max drafts</font></small></td>
                    <td id="maxdfore"><font color=blue></font></td>
                    <td id="maxdmid"><font color=blue></font></td>
                    <td id="maxdaft"><font color=blue>`+data.opensea[0].maxdraft.toFixed(3)+`</font></td>
                </tr>
            </table>
          </div>
